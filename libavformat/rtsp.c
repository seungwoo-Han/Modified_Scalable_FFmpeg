...
#define COMMON_OPTS() \
    ...
    { "scale",        "Play Speed",                  OFFSET(scale),           AV_OPT_TYPE_FLOAT, {.dbl=1.0},-64.0, 64.0, DEC }, \
    { "start_time",        "Start Time",                  OFFSET(start_time),           AV_OPT_TYPE_STRING, {.str = ""}, 0, 0, DEC }, \
    { "end_time",        "End Time",                  OFFSET(end_time),           AV_OPT_TYPE_STRING, {.str = ""}, 0, 0, DEC }, \
...
static AVDictionary *map_to_opts(RTSPState *rt)
{
    ...
    snprintf(buf, sizeof(buf), "%f", rt->scale);
    av_dict_set(&opts, "scale", buf, 0);

    snprintf(buf, sizeof(buf), "%s", rt->start_time);
    av_dict_set(&opts, "start_time", buf, 0);
    snprintf(buf, sizeof(buf), "%s", rt->end_time);
    av_dict_set(&opts, "end_time", buf, 0);
    return opts;
}
                                            ...
                                            int send_content_length)
{
    ...
    if(method == "PLAY" || method == "SETUP")
        av_strlcatf(buf, sizeof(buf), "Require: %s\r\n", "onvif-replay");

    if (method == "PLAY") {
        av_strlcatf(buf, sizeof(buf), "Scale: %f\r\n", rt->scale);
        av_strlcatf(buf, sizeof(buf), "Rate-Control: %s\r\n", "yes");
    }
